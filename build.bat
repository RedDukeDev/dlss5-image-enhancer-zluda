...
cmake --build build
if errorlevel 1 exit /b 1
REM A build from when this program linked Qt6::Sql leaves its DLL and its SQL
REM driver plugins in build\, and windeployqt only ever adds, never removes: the
REM mirror below would carry them into dist\ for good. Nothing links them now.
if exist "build\sqldrivers" rmdir /s /q "build\sqldrivers"
if exist "build\Qt6Sql.dll" del /q "build\Qt6Sql.dll"
